...
const Webhooks = () => {
  const { data: webhooks = [], isLoading, error } = useListWebhooksQuery();
  ...
  const { data: logs = [], isLoading: isLoadingLogs } =
    useGetWebhookLogsQuery(selectedWebhookId, { skip: !showLogsModal });
    ...
    try {
      ...
      try {
        if (currentWebhook.headers) {
          parsedHeaders =
            typeof currentWebhook.headers === "string"
      ...
      if (isEditing && currentWebhook.id) {
        ...
        await createWebhook(payload as Partial<Webhook> & Pick<Webhook, "name" | "url" | "method">).unwrap();
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Manage event callbacks and integrations
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        {error && <div className="text-red-500 mb-4">Failed to load webhooks</div>}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {webhooks.map((hook) => (
              <div
                key={hook.id}
                className="glass-panel p-6 rounded-xl border border-slate-700 bg-slate-800/50 hover:border-cyan-500/50 transition-colors"
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className="text-slate-300 text-sm truncate font-mono"
                      title={hook.url}
                    >
                      {hook.url}
                    </span>
              ...
              {isEditing && currentWebhook.signingSecret && (
                <div className="mb-6 p-4 bg-slate-900 rounded-lg border border-slate-700">
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    Use this secret to verify the authenticity of webhook
                    requests.
                  </p>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={
                      typeof currentWebhook.headers === "string"
                        ? currentWebhook.headers
                        : JSON.stringify(currentWebhook.headers || {}, null, 2)
                    }
                    onChange={(e) =>
                      setCurrentWebhook({
                        ...currentWebhook,
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">
                    ...
                    <span className="text-slate-500 text-xs ml-2">
                      Go template syntax. Leave empty for default JSON.
                    </span>
                  </label>
                  <textarea
                    className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-cyan-500 outline-none font-mono text-sm"
...
const LogsTable = ({ logs }: { logs: WebhookLog[] }) => (
          ...
          <td className="px-4 py-2 text-right">
            <details className="cursor-pointer">
              ...
              <div className="mt-2 p-2 bg-slate-950 rounded text-xs font-mono whitespace-pre-wrap text-left absolute z-10 w-96 right-10 border border-slate-700 shadow-xl">
                <div className="mb-2 border-b border-slate-800 pb-1 font-bold text-slate-300">
                  Request:
                </div>
                <div className="mb-4 text-slate-400 max-h-32 overflow-y-auto">
                  {log.requestBody}
                </div>
                <div className="mb-2 border-b border-slate-800 pb-1 font-bold text-slate-300">
                  Response:
